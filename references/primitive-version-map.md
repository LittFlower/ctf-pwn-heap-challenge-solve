...
- No direct `free`: prioritize top chunk or `sysmalloc`-style paths, but keep the route at `sysmalloc_int_free` until the freed wilderness becomes a reachable chosen-pointer or poisoning bridge.
...
- `house_of_water` or TSU heap fengshui unclear: open `house-water-and-stash-fengshui.md` before guessing offsets, tcache drain counts, or smallbin positions.
...
- Smallbin family unclear: open `smallbin-techniques.md` before deciding whether the first proof target is House of Lore fake return, smallbin unlink write, TSU stash behavior, or backward-consolidation unlink.
...
| UAF on freed tcache chunk without heap leak | `house_of_water`, `tcache_metadata_poisoning`, `tcache_relative_write` | Mostly `2.32+` | Need metadata reach, offset writes, or a tolerable brute-force budget. For `house_of_water`, prove tcache metadata overlap first and re-check `2.42+` metadata placement. |
...
| Unsorted / smallbin / largebin route still ambiguous | `how2heap-bin-attacks.md`, then `smallbin-techniques.md` for same-size smallbin cases, then `how2heap-bin-write-primitives.md` and exact bin file | Broad | Use the repo-backed first cut before deciding whether the first proof target is a write, a chosen-pointer return, or stash-assisted mixed behavior. |
...
| Overflow or UAF on unsorted, large, or small bin metadata | `unsorted_bin_attack`, `large_bin_attack`, `house_of_lore`, `tcache_stashing_unlink_attack`; use `smallbin-techniques.md` when same-size smallbin behavior is central | Version specific | Exact bin state, sort order, tcache drain count, `target+0x18` writability, and whether the writeup's `smallbin attack` or `tcache stash unlink` wording is really describing fake return, unlink write, stash-assisted writeback, a returned fake chunk, or a TSU++ combined write-and-return decide feasibility. |
...
| No `free`, but top chunk size or control exists | `house_of_tangerine`, `sysmalloc_int_free`, old `house_of_force`, `house_of_orange` when the finish is stdio-facing | Modern / pre-`2.29` for the older houses | Require enough allocation volume, usually 0x1000 top-chunk alignment, and a split between "old wilderness merely got freed" (`sysmalloc_int_free`) and "old wilderness became a reachable tcache-poison bridge" (`house_of_tangerine`). On `2.32+`, classic Tangerine also needs a heap leak or another safe-linking-compatible path; on `2.42+`, re-check target-chunk priming and tcache-transfer details against the maintained example. If the Orange-era finish starts collapsing into old `FSOP`, `_IO_list_all`, or `_IO_str_*` wording, reopen `libio-stdio-primitives.md` before treating it as one generic FILE endgame. |
...
| Writable target is really stdin/stdout fields or `_fileno`, not a generic fake-FILE carrier | `libio-stdio-primitives.md`, then `heap-address-leaks.md` or `leak-and-endgame-map.md` | Broad; exact path still depends on the shipped stdio helpers | Split redirected-fd use, stdin-backed write windows, stdout-backed leaks, and historical `_IO_str_*` dispatch before comparing Apple-family or generic exit-linked finishes. |
| Need a modern FSOP or exit-linked finish after arbitrary write or large-bin control | `house of apple2`, `house of cat`, `house of emma`, `house of banana`, `house of kiwi` | Mostly `2.34+` | Choose by trigger surface, FILE-field control, whether `link_map` / fini-style surfaces are already reachable, and whether kiwi is only a trigger helper through the shipped-libc assert path rather than the finish itself. If the source notes really say `_fileno`, stdin/stdout arbitrary read/write, or old `FSOP`, leave this row and open `libio-stdio-primitives.md` first. |
...
4. Choose one candidate technique family. Use `how2heap-selector.md` for the first cut, then the specialized selector that matches the corridor: `how2heap-legacy-writeup-labels.md`, `heap-address-leaks.md`, `offbynull-heap.md`, `how2heap-overlap-nullbyte.md`, `how2heap-mmap-overlap.md`, `how2heap-fake-free-primitives.md`, `how2heap-freelist-primitives.md`, `how2heap-safe-linking.md`, `how2heap-bin-attacks.md`, `smallbin-techniques.md`, `how2heap-bin-write-primitives.md`, `house-water-and-stash-fengshui.md`, or `how2heap-wilderness-and-arena.md`.
