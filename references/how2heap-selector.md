...
| The notes say `smallbin attack` or only describe same-size smallbin `bk` corruption | `how2heap-bin-attacks.md`, then `smallbin-techniques.md` | Whether the first stable result is fake-chunk return, unlink write, stash-assisted movement, or backward-consolidation unlink | Smallbin wording hides several different proof targets; split before coding. |
| The notes say `house of lore` | `how2heap-bin-attacks.md`, then `smallbin-techniques.md`, `how2heap-bin-write-primitives.md`, and `house_of_lore` | Whether the first stable result is a smallbin fake-chunk return rather than a write-first unsorted or largebin route | The spaced label is still a smallbin-return family, not a generic bin attack keyword |
...
- If the source material says `smallbin attack`, open `smallbin-techniques.md` and classify the first proof target before comparing it with generic bin, stash, or unsafe-unlink routes.
- If the source material says `house of lore`, keep it in the bin corridor and open `smallbin-techniques.md` before comparing it against other return-first smallbin or unsorted routes.
...
12. Open `smallbin-techniques.md` when same-size smallbin return, smallbin unlink write, House of Lore, or smallbin-to-tcache stash behavior is the blocker.
13. Open `house-water-and-stash-fengshui.md` when `house_of_water` or TSU/TSU+/TSU++ heap fengshui is the blocker.
14. Open `how2heap-wilderness-and-arena.md` when the route depends on wilderness growth, `sysmalloc`, or fake-arena behavior.
15. Open `how2heap-mmap-overlap.md` when the route depends on mmap-sized allocations, `munmap`, and later huge-allocation reclamation rather than normal bin reuse.
