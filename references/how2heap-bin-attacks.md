...
| Smallbin freelist corruption wants a chosen pointer | `house_of_lore`, then `how2heap-bin-write-primitives.md` | smallbin hardening checks still pass while returning the staged fake chunk |
...
| `calloc` and smallbin-to-tcache stash motion are part of the exploit | `tcache_stashing_unlink_attack`, then `how2heap-bin-write-primitives.md` | stash movement creates both the write and fake return side effect |
